...
def get_vehicle_info_from_url(url: str) -> tuple[str, dict]:
    try:
        ...
        prompt = f"""
        Analyze the car listing from the URL below. 
        URL: {url}
        ...
        4. Price (Integer only, remove '$' and ','. If not found, use 0)

        Return ONLY a raw JSON object with keys: "maker", "model", "year", "price".
        """
        
        # [FIX 1] 모델명 변경 (latest는 불안정함, flash가 빠르고 정확함)
        # [FIX 2] response_mime_type을 사용하여 JSON 응답 강제 (파싱 에러 방지)
        ...
        # JSON 강제 모드이므로 텍스트 정제 불필요
        raw_response_string = response.text
        vehicle_data = json.loads(raw_response_string)
        
        return raw_response_string, vehicle_data

    except Exception as e:
        # 에러 발생 시에도 JSON 구조로 리턴하도록 보장
        return str(e), {"error": f"Gemini API Error: {str(e)}"}
...
def call_real_vehicle_api(vehicle_data: dict) -> dict:
    ...
    # 데이터 안전성 체크
    if not vehicle_data or "model" not in vehicle_data:
        return {"error": "Invalid vehicle data provided."}

    original_model = vehicle_data.get("model", "")
    simplified_model = original_model.split(' ')[0] # 모델명 단순화
    ...
    if not all(params.values()): 
        return {"error": "Maker, model, and year are required for valuation."}
    ...
    try:
        ...
        if not data_payload.get('success') or data_payload.get('error') == 'no_data':
            return {"error": f"No market data found for this car.", "raw_api_response": api_data}
            
        valuation_price = data_payload.get('mean')
        
        if valuation_price is None:
            return {"error": "Market data found but price is missing.", "raw_api_response": api_data}
...
def calculate_deal_rating(listing_price: int, valuation_price: int) -> dict:
    ...
    price_diff_str = f"${abs(difference):,}"
